...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //Object created for UserDefaults
        let defaults = UserDefaults.standard
        //Code to fetch from UserDefaults if saved data exists
        if let savedPeople = defaults.object(forKey: "people") as? Data {
            //Fetch data with NSKeyedUnarchiver
    ...
    //Function to save the data in people object to UserDefaults.
    func save() {
        //Create NSKeyedArchiver from people object
        if let savedData = try? NSKeyedArchiver.archivedData(withRootObject: people, requiringSecureCoding: false) {
            let defaults = UserDefaults.standard
            //Set the achived data in UserDefaults
